...
- **SSH**: Modular `config.d/` setup with multiplexing, keepalive, and
  hardened defaults (agent forwarding off globally, per-host opt-in)
...
Focused guides live in `docs/`:

- [docs/daily-maintenance.md](docs/daily-maintenance.md) — update
  automation: schedule, control commands, configuration, logs
- [docs/keybindings.md](docs/keybindings.md) — keybinding reference
...
Homebrew, zinit, Oh-My-Zsh, bob, LazyVim, and treesitter update
themselves at 9:00 AM via launchd, with a catch-up run at login if the
laptop was off. Quick aliases: `mr` (run), `ms` (status), `ml` (logs).

Install, control commands, scheduling, logs, troubleshooting, and
uninstall: see [docs/daily-maintenance.md](docs/daily-maintenance.md).
...
- **Agent Forwarding**: `ForwardAgent no` globally (a compromised
  server could otherwise use your keys) — opt in per-host for trusted
  machines with a `Host <name>` block setting `ForwardAgent yes`
- **Hardened defaults**: `IdentitiesOnly yes`,
...
remote Linux server): [docs/herdr-setup.md](docs/herdr-setup.md).

## 🖥️ Tmux Configuration

### Setup

TPM (Tmux Plugin Manager) is git-cloned to `~/.tmux/plugins/tpm` by the
bootstrap script (it is not a Homebrew package). After installing dotfiles:
...
| - | **jjui** | TUI for jujutsu | `jjui` |
| `nvm`/`pyenv` | **mise** | Polyglot version manager | `mise` |
| `make` | **just** | Command runner with simple justfiles | `just` |
| - | **tailspin** | Zero-config log highlighter/pager | `tspin` |
| `tar`/`unzip` | **ouch** | Universal compression/extraction | `extract`, `x` |
| `diff` | **difftastic** | Syntax-aware semantic diff | `git dft` |
...
- **Archive extraction**: `extract` / `x` aliases run `ouch decompress`
  (replaces the OMZ extract plugin; multi-file archives land in a
  subdirectory named after the archive)
- **Command correction**: `fk` runs `pay-respects` (Rust successor to
  thefuck, installed via cargo by the bootstrap script)
- **Content search**: television is the full-text content search entry
  point — `tvt` is an alias for `tv text`; fzf owns the filename and
  history flows
...
│   ├── git/config     # Shared git config (tracked; ~/.gitconfig is
...
│       └── hooks/
│           └── pre_commit  # Pre-commit validation (yadm 3.x path)
├── .claude/           # Claude Code shared settings, skills, theme
├── .ssh/
...
├── test-bootstrap.sh               # Bootstrap test suite
├── .github/
│   └── workflows/
